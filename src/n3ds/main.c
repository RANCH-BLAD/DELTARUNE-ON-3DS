...
// Route the VM's obj_initializer2 trace to the SD breadcrumb log.
// Lifecycle breadcrumbs (boot/room/gfx stages) stay enabled for crash forensics.
// VM call tracing: ring-buffered in RAM (batched SD flush, marker rewritten per
// call) so the exact final VM call survives a hard crash without SD thrashing.
void VMExec_platformBootLog(const char* message) {
    if (message != NULL) N3DSDebugLog_ringWrite("vmbt", "%s", message);
}
bool VMExec_shouldTraceCrashWindow(void) {
    // SPEED FIX: per-CallV/vmarray tracing was a runtime firehose (marker SD
    // rewrite per call + batched flushes) and dragged the game below playable.
    // Crash forensics now live in the tdn/room/boot/audio events (rare, cheap)
    // — per-VM-call traces stay compiled but dormant.
    return false;
}
...
// ===[ System event ring: RAM-only, painted on the bottom-screen HUD ]===
#define N3DS_SYSLOG_LINES 7u
#define N3DS_SYSLOG_LINE_LEN 80u
static char gSysLog[N3DS_SYSLOG_LINES][N3DS_SYSLOG_LINE_LEN];
static uint32_t gSysLogHead = 0;   // next write slot
static uint32_t gSysLogCount = 0;  // total writes (for wrap awareness)

void N3DS_SysLog(const char* fmt, ...) {
    char line[N3DS_SYSLOG_LINE_LEN];
    va_list args;
    va_start(args, fmt);
    vsnprintf(line, sizeof(line), fmt, args);
    va_end(args);
    snprintf(gSysLog[gSysLogHead], N3DS_SYSLOG_LINE_LEN, "%s", line);
    gSysLogHead = (gSysLogHead + 1u) % N3DS_SYSLOG_LINES;
    gSysLogCount++;
}
void N3DS_SysLog_dump(void (*sink)(const char*)) {
    if (sink == NULL) return;
    static char scratch[N3DS_SYSLOG_LINE_LEN];
    uint32_t start = (gSysLogCount >= N3DS_SYSLOG_LINES)
        ? gSysLogHead
        : 0u;
    for (uint32_t k = 0; k < N3DS_SYSLOG_LINES && k < gSysLogCount; k++) {
        uint32_t idx = (start + k) % N3DS_SYSLOG_LINES;
        sink(gSysLog[idx]);
    }
}
...
static void N3DSDebugMonitor_draw(N3DSDebugMonitor* monitor, Runner* runner, Renderer* renderer) {
    ...
    {
        extern void N3DS_SysLog_dump(void (*sink)(const char*));
        // Ring buffer rows under the stat rows.
        uint32_t k = 0;
        // dump needs a callback; simple re-implementation here to keep rows inline:
        uint32_t count = gSysLogCount, head = gSysLogHead;
        uint32_t start = (count >= N3DS_SYSLOG_LINES) ? head : 0u;
        for (k = 0; k < N3DS_SYSLOG_LINES; k++) {
            if (k >= count) break;
            uint32_t idx = (start + k) % N3DS_SYSLOG_LINES;
            N3DSDebugTinyFont_drawText(gSysLog[idx], textX, boxY + 114.0f + 15.0f * (float) k, C2D_Color32(150, 205, 150, 255));
        }
    }
...
static bool N3DS_tryLoadRoomBorderSprite(const char* assetName, C2D_Sprite* outSprite, C2D_SpriteSheet* outSheet) {
    ...
    snprintf(candidatePaths[0], sizeof(candidatePaths[0]), "sdmc:/3ds/papyrus/gfx/borders/%s.t3x", assetName);
...
static char* chooseDataWinPath(void) {
    ...
    return safeStrdup("sdmc:/3ds/papyrus/data.win");
}
...
// ===[ Big-stack VM worker ]===
// VM_callCodeIndex alone consumes ~2.5KB of stack per GML call level; with the
// event-dispatch frames on top, deep-but-legitimate GML recursion (Deltarune's
// event manager) blows the 3DSX default stack within a handful of levels.
// The whole VM main-loop therefore runs on a dedicated thread with a 1MB
// stack. Main thread: init, spawn worker, join, teardown.
#define N3DS_VM_STACK_SIZE (1024u * 1024u)
#define N3DS_VM_THREAD_PRIORITY 0x3F

static Thread N3DS_vmThread = NULL;
static volatile bool N3DS_vmThreadDone = false;

int N3DS_vmMain(int argc, char** argv);

static void N3DS_vmThreadMain(void* arg) {
    (void) arg;
    int rc = N3DS_vmMain(0, NULL);
    (void) rc;
    N3DS_vmThreadDone = true;
}
...
int N3DS_vmMain(int argc, char** argv) {
    ...
    if (dataWin == NULL) {
        ...
        N3DS_SysLog("CRASH: data.win failed to load");
        N3DSLoadingScreen_free(&loadingScreen);
        return 1;
    }

    FileSystem* fileSystem = (FileSystem*) N3DSFileSystem_create("romfs:/", "sdmc:/3ds/papyrus/");
    N3DSDebugLog_installCrashHandler();
    ...
    N3DSDebugLog_event("gfx", "audio vtable init returned");
    N3DS_SysLog("AUDIO: bank loaded");
    ...
    N3DSDebugLog_event("boot", "runner created, roomOrder0=%ld roomCount=%lu", (long) dataWin->gen8.roomOrder[0], (unsigned long) dataWin->room.count);
    ...
    N3DSDebugLog_event("boot", "loading first room");
    N3DS_SysLog("BOOT: loading first room");
    Runner_initFirstRoom(runner);
    N3DSDebugLog_event("boot", "first room loaded roomIndex=%ld name=%s", (long) runner->currentRoomIndex, (runner->currentRoomIndex >= 0 && (uint32_t) runner->currentRoomIndex < dataWin->room.count) ? dataWin->room.rooms[runner->currentRoomIndex].name : "?");
    ...
    while (aptMainLoop() && !runner->shouldExit) {
        ...
        if (runner->pendingRoom >= 0) {
            N3DS_SysLog("ROOMGOTO -> room %ld now=%ld", (long) runner->pendingRoom, (long) runner->currentRoomIndex);
            N3DSDebugLog_event("frame", "ROOMGOTO pending=%ld now=%ld", (long) runner->pendingRoom, (long) runner->currentRoomIndex);
            framesAfterRoomChange = 0;
        }
        if (runner->shouldExit) N3DS_SysLog("GAME REQUESTED EXIT (room_goto menu/quit)");
        ...
        if (framesAfterRoomChange < 3 && runner->pendingRoom < 0) {
            framesAfterRoomChange++;
            N3DSDebugLog_event("frame", "frame alive #%d room=%ld", framesAfterRoomChange, (long) runner->currentRoomIndex);
        }
        N3DS_refreshRoomBorderSprite(runner->currentRoom, &roomBorder, &roomBorderSheet, &haveRoomBorder, loadedRoomBorderAsset, sizeof(loadedRoomBorderAsset));
...
// Real entry: set up gfx/romfs on the main thread, then run the entire VM
// loop (N3DS_vmMain) on a worker thread with a 1MB stack. Deep-but-legitimate
// GML recursion (Deltarune event dispatch) needs far more stack than the
// 3DSX default; see the N3DS_vmStack comment above.
int main(int argc, char** argv) {
    ...
    N3DS_vmThread = threadCreate(
        N3DS_vmThreadMain,
        NULL,
        N3DS_VM_STACK_SIZE,
        N3DS_VM_THREAD_PRIORITY,
        -1,
        false
    );
    if (N3DS_vmThread == NULL) {
        N3DSDebugLog_event("boot", "FATAL: failed to create VM thread");
        C2D_Fini();
        C3D_Fini();
        romfsExit();
        gfxExit();
    ...
    // Main thread must keep servicing APT while the worker runs — a blocked
    // threadJoin suspends the process and the worker hangs on GPU/service
    // calls (seen as "stuck at LOADING GAME DATA").
    while (!N3DS_vmThreadDone) {
        aptMainLoop();
        svcSleepThread(16 * 1000 * 1000ULL);
    }
    threadJoin(N3DS_vmThread, U64_MAX);
    threadFree(N3DS_vmThread);
    N3DS_vmThread = NULL;
