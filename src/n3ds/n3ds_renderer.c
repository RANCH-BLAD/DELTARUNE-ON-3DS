...
#define N3DS_SDMC_ASSET_BASE "sdmc:/3ds/papyrus/gfx"
...
#define N3DS_ATLAS_TRACE_LOG_PATH "sdmc:/3ds/papyrus/atlas_trace.log"
...
static bool N3DSRenderer_loadAtlas(N3DSRenderer* renderer) {
    ...
    if (atlasFile == NULL) {
        ...
        fprintf(stderr, "N3DS: missing atlas.bin in sdmc:/3ds/papyrus/gfx or romfs:/gfx\n");
        return false;
    }
...
static void N3DSRenderer_prewarmRoomBlobCache(N3DSRenderer* renderer, Runner* runner) {
    ...
    N3DSDebugLog_event("prewarm", "blob walk begin room=%lu pages=%lu blobBudgetKB=%lu",
        (unsigned long) roomIndex, (unsigned long) renderer->atlasPageCount, (unsigned long) (remainingBlobBytes / 1024u));

    if (roomIndex != UINT32_MAX) {
        N3DSRenderer_prewarmRoomManifestBlobCache(renderer, roomIndex, seenPages, &remainingBlobBytes);
        N3DSDebugLog_event("prewarm", "manifest stage done remKB=%lu", (unsigned long) (remainingBlobBytes / 1024u));
    ...
    N3DSDebugLog_event("prewarm", "backgrounds stage done remKB=%lu", (unsigned long) (remainingBlobBytes / 1024u));
    ...
    N3DSDebugLog_event("prewarm", "layers stage done remKB=%lu", (unsigned long) (remainingBlobBytes / 1024u));
    ...
    N3DSDebugLog_event("prewarm", "tiles stage done remKB=%lu", (unsigned long) (remainingBlobBytes / 1024u));
    ...
    free(seenPages);
    N3DSDebugLog_event("prewarm", "blob walk done remKB=%lu", (unsigned long) (remainingBlobBytes / 1024u));
}
...
static void N3DSRenderer_init(Renderer* base, DataWin* dataWin) {
    N3DSDebugLog_event("gfx", "renderer init begin");
    N3DSRenderer* renderer = (N3DSRenderer*) base;
    bool isNew3DS = false;
    ...
    N3DSDebugLog_event("gfx", "atlas load done loaded=%d pages=%u items=%lu frags=%lu", (int) renderer->atlasLoaded, renderer->atlasPageCount, (unsigned long) renderer->atlasItemCount, (unsigned long) renderer->atlasFragmentCount);
    ...
    N3DSDebugLog_event("gfx", "direct asset maps built");
    (void) N3DSRenderer_loadPackedDirectAssets(renderer);
    N3DSDebugLog_event("gfx", "packed direct assets loaded");
    (void) N3DSRenderer_loadRoomManifest(renderer);
    N3DSDebugLog_event("gfx", "room manifest loaded");
    ...
    N3DSDebugLog_event("gfx", "font pages preloaded");
