...
// Per-builtin-call tracing flood gate: the 661s forensic run produced 20k+
// "vmbuiltin:" lines because every draw_text/string_hash_to_newline from ANY
// object traced. That flood buried the real signal and burned SD writes.
// Restrict per-call tracing to the crash-window objects (obj_time, the intro
// typewriter whose teardown died, and obj_initializer2). Room/boot/audio
// breadcrumbs (tdn/audio_stop) are NOT affected — they stay unconditional.
static bool VMExec_shouldTraceBuiltinCall(VMContext* ctx) {
    if (!VMExec_shouldTraceCrashWindow()) return false;
    return VMExec_shouldTraceBootstrapObject(ctx, NULL);
}
...
static void handleCall(VMContext* ctx, uint32_t instr, const uint8_t* extraData) {
    ...
    if (cache->builtin != nullptr) {
        ...
        if (VMExec_shouldTraceBuiltinCall(ctx)) {
            char buffer[256];
            snprintf(
                buffer,
                sizeof(buffer),
...
static void handleCallV(VMContext* ctx, uint32_t instr) {
    ...
    if (VMExec_shouldTraceCrashWindow()) {
        ...
        // Route through the platform ring (survives hard crash) — stderr is
        // invisible on retail 3DS, which hid CALLV deaths from forensics.
        VMExec_bootLog(vmcallvBuf);
