...
#ifdef __3DS__
// N3DS QUIET EXPERIMENT: printf/fprintf to unattached stdout/stderr on retail
// 3DS = _vfprintf_r on a broken FILE* = _dtoa_r death (dump 13 signature).
// All diagnostics go through the n3ds ring logger instead.
...
static void initRoom(Runner* runner, int32_t roomIndex) {
    ...
    N3DSDebugLog_event("room", "initRoom begin room=%ld name=%s", (long) roomIndex, dataWin->room.rooms[roomIndex].name);
    ...
    N3DSDebugLog_event("room", "payload loaded");
    ...
    N3DSDebugLog_event("room", "pass1 done instances=%lu objects=%lu", (unsigned long) arrlen(runner->instances), (unsigned long) room->gameObjectCount);
    ...
    repeat(room->gameObjectCount, i) {
        ...
        N3DSDebugLog_event("room", "create events: inst %lu/%lu objDef=%ld", (unsigned long) i, (unsigned long) room->gameObjectCount, (long) roomObj->objectDefinition);
    ...
    N3DSDebugLog_event("room", "pass2 create events done");
    ...
    N3DSDebugLog_event("room", "room creation code done, prewarming");
    ...
    N3DSDebugLog_event("room", "renderer prewarm begin");
    phaseStartMs = Runner_nowMs();
    if (runner->renderer != nullptr && runner->renderer->vtable->prewarmRoom != nullptr) {
        runner->renderer->vtable->prewarmRoom(runner->renderer, runner);
    }
    Runner_logTiming("renderer prewarm", phaseStartMs, roomStartMs);
    N3DSDebugLog_event("room", "renderer prewarm done");
    phaseStartMs = Runner_nowMs();
    N3DSDebugLog_event("room", "audio prewarm begin");
    if (runner->audioSystem != nullptr && runner->audioSystem->vtable->prewarmRoom != nullptr) {
        runner->audioSystem->vtable->prewarmRoom(runner->audioSystem, runner);
    }
    Runner_logTiming("audio prewarm", phaseStartMs, roomStartMs);
    N3DSDebugLog_event("room", "audio prewarm done, room init complete");
}
...
void Runner_initFirstRoom(Runner* runner) {
    ...
    fprintf(stderr, "Runner: Glob init scripts: %u total\n", dataWin->glob.count);
    N3DSDebugLog_event("boot", "global init scripts begin count=%lu", (unsigned long) dataWin->glob.count);
    repeat(dataWin->glob.count, i) {
        ...
        if (codeId >= 0 && dataWin->code.count > (uint32_t) codeId) {
            ...
            N3DSDebugLog_event("boot", "glob script %lu/%lu id=%ld name=%s", (unsigned long) i, (unsigned long) dataWin->glob.count, (long) codeId, dataWin->code.entries[codeId].name);
            RValue result = VM_executeCode(runner->vmContext, codeId);
            RValue_free(&result);
        }
    }
    runner->vmContext->currentInstance = nullptr;
    N3DSDebugLog_event("boot", "global init scripts done");
